...
class UpsamplingAdd(nn.Module):
    ...
    def forward(self, x, x_skip):
        ...
        #     if self.key == 256:
        #         x = (x-norm_mask['norm_feat_2_mean']) / (norm_mask['norm_feat_2_var']+1e-5)
        #     if self.key == 128:
        #         x = (x-norm_mask['norm_feat_3_mean']) / (norm_mask['norm_feat_3_var']+1e-5)
        #     if self.key == 64:
        #         x = (x-norm_mask['norm_feat_4_mean']) / (norm_mask['norm_feat_4_var']+1e-5)
        # else :
        #     mean = torch.mean(x, dim=(2,3), keepdim=True)
        #     var = torch.var(x, dim=(2,3), unbiased=True, keepdim=True)
...
class Segnet_relu(nn.Module):
    def __init__(self, Z, Y, X, vox_util=None, 
                 ...
                 norm_mask=None,
                 encoder_type="res101"):
        super(Segnet_relu, self).__init__()
    ...
    def forward(self, rgb_camXs, pix_T_cams, cam0_T_camXs, vox_util, rad_occ_mem0=None):
        ...
        if self.use_radar:
            ...
            # pdb.set_trace()
            assert(rad_occ_mem0 is not None)
            feat_bev_ = feat_mem.permute(0, 1, 3, 2, 4).reshape(B, self.feat2d_dim*Y, Z, X) # (1, 1024, 200, 200)
            rad_bev_ = rad_occ_mem0.permute(0, 1, 3, 2, 4).reshape(B, Y, Z, X) # (1, 8, 200, 200)
            
            feat_bev_ = torch.cat([feat_bev_, rad_bev_], dim=1) # (1, 1032, 200, 200)
            feat_bev_ = self.bev_compressor(feat_bev_)
        ...
        return act_e, norm_e, seg_e, center_e, offset_e
