...
class UpsamplingAdd(nn.Module):
    ...
    def forward(self, cam_bev_, x_skip):
        # cam_bev_ = self.upsample_layer(cam_bev_)
        # mean = 0
        # var = 0
        # if norm_mask is not None :
        #     if self.key == 256:
        #         cam_bev_ = (cam_bev_-norm_mask['norm_feat_2_mean']) / (norm_mask['norm_feat_2_var']+1e-5)
        #     if self.key == 128:
        #         cam_bev_ = (cam_bev_-norm_mask['norm_feat_3_mean']) / (norm_mask['norm_feat_3_var']+1e-5)
        #     if self.key == 64:
        #         cam_bev_ = (cam_bev_-norm_mask['norm_feat_4_mean']) / (norm_mask['norm_feat_4_var']+1e-5)
        # else :
        #     mean = torch.mean(cam_bev_, dim=(2,3), keepdim=True)
        #     var = torch.var(cam_bev_, dim=(2,3), unbiased=True, keepdim=True)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, n_classes, predict_future_flow):
        super().__init__()
        #self.splitting()
        self.splitting_rule = "rule_1"
        self.norm_bias_rule =  "ratio" #"ratio" # option : ["uniform", "ratio", "identity", "grad"]
    ...
    def splitting_2(self, cam_feat, rad_feat, pseudo_feat):
        ...
        # sum_condition_cam = ((cam_feat > 0) & (rad_feat <= 0) & (pseudo_feat > 0))
        # sum_condition_rad = ((cam_feat <= 0) & (rad_feat > 0) & (pseudo_feat > 0))
        # sum_condition_rad_2 = ((cam_feat <= 0)) & (rad_feat > 0) & (pseudo_feat <= 0)
        ...
        # output_feat_1 = torch.where(sum_condition_rad_2, rad_feat, output_feat_1)
        # output_feat_2 = torch.where(sum_condition_rad_2, cam_feat, output_feat_2)
        # output_feat_3 = torch.where(sum_condition_rad_2, pseudo_feat, output_feat_3)
        
        # if self.conservation_bias_split : 
        #     #pdb.set_trace()
        #     assert torch.allclose(output_feat_1 + output_feat_2 + output_feat_3, cam_feat + rad_feat + pseudo_feat ,atol=1e-02, rtol=1e-05
        #     ), f"Conservation Mismatch_3"
        
        return output_feat_1, output_feat_2, output_feat_3
    ...
    def splitting(self, cam_feat, rad_feat, pseudo_feat):
        ...
        if check == "ratio" :
            ...
            # sum_condition_cam = ((cam_feat > 0) & (rad_feat <= 0) & (pseudo_feat > 0)).any() or ((cam_feat < 0) & (rad_feat >= 0) & (pseudo_feat < 0))
            # sum_condition_rad = ((cam_feat <= 0) & (rad_feat > 0) & (pseudo_feat > 0)).any() or ((cam_feat >= 0) & (rad_feat < 0) & (pseudo_feat < 0))
        ...
        # if self.conservation_bias_split : 
        #     #pdb.set_trace()
        #     assert torch.allclose(output_feat_1 + output_feat_2 + output_feat_3, cam_feat + rad_feat + pseudo_feat ,atol=1e-02, rtol=1e-02), f"Conservation Mismatch_3"
            
        return output_feat_1, output_feat_2, output_feat_3
    
    
    def forward(self, feat_bev_, cam_bev_, rad_bev_, pseudo_bev_, bev_flip_indices=None, act_mask=None, norm_mask=None):
        ...
        #     assert torch.allclose(feat_bev_, cam_bev_ + rad_bev_ + pseudo_bev_, atol=1e-02), f"Conservation Mismatch_2"
        ...
        # error 값이 1e+03 만큼 critical하게 뛰는 부분이다.
        ...
        if self.norm_bias_rule_bn == "uniform" : 
            ...
            cam_bev_ = (cam_bev_ - self.bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.bn1.running_var[None, :, None, None] + self.bn1.eps)
            cam_bev_ = cam_bev_ * self.bn1.weight[None, :, None, None] + self.bn1.bias[None, :, None, None] / 3
            
            rad_bev_ = (rad_bev_ - self.bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.bn1.running_var[None, :, None, None] + self.bn1.eps)
            rad_bev_ = rad_bev_ * self.bn1.weight[None, :, None, None] + self.bn1.bias[None, :, None, None] / 3
            
            pseudo_bev_ = (pseudo_bev_ - self.bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.bn1.running_var[None, :, None, None] + self.bn1.eps)
            pseudo_bev_ = pseudo_bev_ * self.bn1.weight[None, :, None, None] + self.bn1.bias[None, :, None, None] / 3
        ...
        # nan value detected
        # if torch.allclose(feat_bev_, feat_bev_2, atol=1e-02, rtol=1e-05) == False:
        #     pdb.set_trace()
        
        if self.splitting_rule == "rule_1" :
            rad_bev_, cam_bev_, pseudo_bev_ = self.splitting(cam_bev_, rad_bev_, pseudo_bev_)

        elif self.splitting_rule == "rule_2" :
            rad_bev_, cam_bev_, pseudo_bev_ = self.splitting_2(cam_bev_, rad_bev_, pseudo_bev_)


        # assert torch.allclose(feat_bev_, cam_bev_ + rad_bev_, atol=1e-02), f"Conservation Mismatch_2"
        ...
        # if torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-02, rtol=1e-2) == False :
        #     print("check for layer1_1_conv1")
        #     pdb.set_trace()
        ...
        if self.norm_bias_rule_bn == "uniform" :  
            # pdb.set_trace()
            out_feat = self.layer1_1_bn1(out_feat)
            #out_feat = (out_feat - self.layer1_1_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer1_1_bn1.running_var[None, :, None, None] + self.layer1_1_bn1.eps)
            out_cam = (out_cam - self.layer1_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_1_bn1.running_var[None, :, None, None] + self.layer1_1_bn1.eps)
            out_cam = out_cam * self.layer1_1_bn1.weight[None, :, None, None] + self.layer1_1_bn1.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer1_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_1_bn1.running_var[None, :, None, None] + self.layer1_1_bn1.eps)
            out_rad = out_rad * self.layer1_1_bn1.weight[None, :, None, None] + self.layer1_1_bn1.bias[None, :, None, None] / 3
            
            out_psd = (out_psd - self.layer1_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_1_bn1.running_var[None, :, None, None] + self.layer1_1_bn1.eps)
            out_psd = out_psd * self.layer1_1_bn1.weight[None, :, None, None] + self.layer1_1_bn1.bias[None, :, None, None] / 3
            ...
            print("lets start !")
        # out_feat = self.layer1_1_bn1(out_feat) 
        # out_cam = self.layer1_1_bn1(out_cam)
        # out_rad = self.layer1_1_bn1(out_rad)
        # out_psd = self.layer1_1_bn1(out_psd)
        
        if self.splitting_rule == "rule_1" : 
            out_rad, out_cam, out_psd = self.splitting(out_cam, out_rad, out_psd)
        elif self.splitting_rule == "rule_2":
            out_rad, out_cam, out_psd = self.splitting_2(out_cam, out_rad, out_psd)
                    
        # if self.conservation_act_sum:
        #     if not torch.allclose(out_feat, out_cam + out_rad, atol=1e-02, rtol=1e-05):
        #         pdb.set_trace()
        # elif self.conservation_feat_sum : 
        #     if not torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-02, rtol=1e-02):
        #         pdb.set_trace()
        ...
        if self.norm_bias_rule_bn == "uniform" :
            #out_feat = (out_feat - self.layer1_1_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer1_1_bn2.running_var[None, :, None, None] + self.layer1_1_bn2.eps)
            out_cam = (out_cam - self.layer1_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_1_bn2.running_var[None, :, None, None] + self.layer1_1_bn2.eps)
            out_cam = out_cam * self.layer1_1_bn2.weight[None, :, None, None] + self.layer1_1_bn2.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer1_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_1_bn2.running_var[None, :, None, None] + self.layer1_1_bn2.eps)
            out_rad = out_rad * self.layer1_1_bn2.weight[None, :, None, None] + self.layer1_1_bn2.bias[None, :, None, None] / 3
            
            out_psd = (out_psd - self.layer1_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_1_bn2.running_var[None, :, None, None] + self.layer1_1_bn2.eps)
            out_psd = out_psd * self.layer1_1_bn2.weight[None, :, None, None] + self.layer1_1_bn2.bias[None, :, None, None] / 3
        ...
        # out_cam = self.layer1_1_bn2(out_cam)
        # out_rad = self.layer1_1_bn2(out_rad)
        # out_psd = self.layer1_1_bn2(out_psd)
        # if self.conservation_feat_sum : 
        #     if not torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-02, rtol=1e-02):
        #         pdb.set_trace()
        ...
        if act_mask is not None:
            ...
            cam_bev_ = self.layer1_1_relu(out_cam)
            self.activ_buffer.append(cam_bev_ > 0)
        ...
        # if self.conservation_act_sum:
        #     if not torch.allclose(feat_bev_, cam_bev_ + rad_bev_, atol=1e-02, rtol=1e-05):
        #         pdb.set_trace()
        # elif self.conservation_feat_sum : 
        #     if not torch.allclose(feat_bev_, cam_bev_ + rad_bev_ + psd_bev_, atol=1e-02, rtol=1e-02):
        #         pdb.set_trace()
        ...
        if self.norm_bias_rule_bn == "uniform" : 
            #out_feat = (out_feat - self.layer1_2_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer1_2_bn1.running_var[None, :, None, None] + self.layer1_2_bn1.eps)
            out_cam = (out_cam - self.layer1_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_2_bn1.running_var[None, :, None, None] + self.layer1_2_bn1.eps)
            out_cam = out_cam * self.layer1_2_bn1.weight[None, :, None, None] + self.layer1_2_bn1.bias[None, :, None, None] / 3
            
            out_rad = (out_rad - self.layer1_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_2_bn1.running_var[None, :, None, None] + self.layer1_2_bn1.eps)
            out_rad = out_rad * self.layer1_2_bn1.weight[None, :, None, None] + self.layer1_2_bn1.bias[None, :, None, None] / 3
            
            out_psd = (out_psd - self.layer1_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_2_bn1.running_var[None, :, None, None] + self.layer1_2_bn1.eps)
            out_psd = out_psd * self.layer1_2_bn1.weight[None, :, None, None] + self.layer1_2_bn1.bias[None, :, None, None] / 3
        ...
        if self.norm_bias_rule_bn == "identity" : 
            ...
            out_psd = (out_psd - self.layer1_2_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer1_2_bn1.running_var[None, :, None, None] + self.layer1_2_bn1.eps)
            out_psd = out_psd * self.layer1_2_bn1.weight[None, :, None, None] + self.layer1_2_bn1.bias[None, :, None, None]
        # out_cam = self.layer1_2_bn1(out_cam)
        # out_rad = self.layer1_2_bn1(out_rad)
        # out_psd = self.layer1_2_bn1(out_psd)
        ...
        if self.norm_bias_rule_bn == "uniform" :
            #out_feat = (out_feat - self.layer1_2_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer1_2_bn2.running_var[None, :, None, None] + self.layer1_2_bn2.eps)
            out_cam = (out_cam - self.layer1_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_2_bn2.running_var[None, :, None, None] + self.layer1_2_bn2.eps)
            out_cam = out_cam * self.layer1_2_bn2.weight[None, :, None, None] + self.layer1_2_bn2.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer1_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_2_bn2.running_var[None, :, None, None] + self.layer1_2_bn2.eps)
            out_rad = out_rad * self.layer1_2_bn2.weight[None, :, None, None] + self.layer1_2_bn2.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer1_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer1_2_bn2.running_var[None, :, None, None] + self.layer1_2_bn2.eps)
            out_psd = out_psd * self.layer1_2_bn2.weight[None, :, None, None] + self.layer1_2_bn2.bias[None, :, None, None] / 3
        ...
        if self.norm_bias_rule_bn == "identity" :
            ...
            out_psd = (out_psd - self.layer1_2_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer1_2_bn2.running_var[None, :, None, None] + self.layer1_2_bn2.eps)
            out_psd = out_psd * self.layer1_2_bn2.weight[None, :, None, None] + self.layer1_2_bn2.bias[None, :, None, None]
        # out_cam = self.layer1_2_bn2(out_cam)
        # out_rad = self.layer1_2_bn2(out_rad)
        # out_psd = self.layer1_2_bn2(out_psd)

        # diff = out_feat - (out_cam + out_rad + out_psd)
        # if not torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-02, rtol=1e-05
        # ):
        #     idx = (diff.abs() > 1e-4).nonzero(as_tuple=True)
        #     print(f"Large difference at indices {idx}: {diff[idx]}")
        #     raise AssertionError(f"Conservation Mismatch_2, large differences at indices {idx}")
        ...
        #         pdb.set_trace()
        # elif self.conservation_feat_sum : 
        #     if not torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-01, rtol=1e-02):
        #         pdb.set_trace()
        ...
        if self.norm_bias_rule_bn == "uniform" : 
            #out_feat = (out_feat - self.layer2_1_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_1_bn1.running_var[None, :, None, None] + self.layer2_1_bn1.eps)
            out_cam = (out_cam - self.layer2_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_bn1.running_var[None, :, None, None] + self.layer2_1_bn1.eps)
            out_cam = out_cam * self.layer2_1_bn1.weight[None, :, None, None] + self.layer2_1_bn1.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer2_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_bn1.running_var[None, :, None, None] + self.layer2_1_bn1.eps)
            out_rad = out_rad * self.layer2_1_bn1.weight[None, :, None, None] + self.layer2_1_bn1.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer2_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_bn1.running_var[None, :, None, None] + self.layer2_1_bn1.eps)
            out_psd = out_psd * self.layer2_1_bn1.weight[None, :, None, None] + self.layer2_1_bn1.bias[None, :, None, None] / 3
        ...
        # out_cam = self.layer2_1_bn1(out_cam)
        # out_rad = self.layer2_1_bn1(out_rad)
        # out_psd = self.layer2_1_bn1(out_psd)
        ...
        if self.norm_bias_rule_bn == "uniform" :
            #out_feat = (out_feat - self.layer2_1_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_1_bn2.running_var[None, :, None, None] + self.layer2_1_bn2.eps)
            out_cam = (out_cam - self.layer2_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_bn2.running_var[None, :, None, None] + self.layer2_1_bn2.eps)
            out_cam = out_cam * self.layer2_1_bn2.weight[None, :, None, None] + self.layer2_1_bn2.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer2_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_bn2.running_var[None, :, None, None] + self.layer2_1_bn2.eps)
            out_rad = out_rad * self.layer2_1_bn2.weight[None, :, None, None] + self.layer2_1_bn2.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer2_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_bn2.running_var[None, :, None, None] + self.layer2_1_bn2.eps)
            out_psd = out_psd * self.layer2_1_bn2.weight[None, :, None, None] + self.layer2_1_bn2.bias[None, :, None, None] / 3
        ...
        if self.norm_bias_rule_bn == "identity" :
            ...
            out_psd = (out_psd - self.layer2_1_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_1_bn2.running_var[None, :, None, None] + self.layer2_1_bn2.eps)
            out_psd = out_psd * self.layer2_1_bn2.weight[None, :, None, None] + self.layer2_1_bn2.bias[None, :, None, None]
            # out_cam = self.layer2_1_bn2(out_cam)
        # out_rad = self.layer2_1_bn2(out_rad)
        # out_psd = self.layer2_1_bn2(out_psd)
        ...
        # identity_feat = self.layer2_1_downsample(feat_bev_)
        # identity_cam = self.layer2_1_downsample(cam_bev_)
        # identity_rad = self.layer2_1_downsample(rad_bev_)
        # identity_psd = self.layer2_1_downsample(psd_bev_)
        
        identity_feat = self.layer2_1_downsample_bn(identity_feat)
        
        if self.norm_bias_rule_bn == "uniform" : 
        #identity_feat = (identity_feat - self.layer2_1_downsample_bn.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_1_downsample_bn.running_var[None, :, None, None] + self.layer2_1_downsample_bn.eps)
            identity_cam = (identity_cam - self.layer2_1_downsample_bn.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_downsample_bn.running_var[None, :, None, None] + self.layer2_1_downsample_bn.eps)
            identity_cam = identity_cam * self.layer2_1_downsample_bn.weight[None, :, None, None] + self.layer2_1_downsample_bn.bias[None, :, None, None] / 3

            identity_rad = (identity_rad - self.layer2_1_downsample_bn.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_downsample_bn.running_var[None, :, None, None] + self.layer2_1_downsample_bn.eps)
            identity_rad = identity_rad * self.layer2_1_downsample_bn.weight[None, :, None, None] + self.layer2_1_downsample_bn.bias[None, :, None, None] / 3

            identity_psd = (identity_psd - self.layer2_1_downsample_bn.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_1_downsample_bn.running_var[None, :, None, None] + self.layer2_1_downsample_bn.eps)
            identity_psd = identity_psd * self.layer2_1_downsample_bn.weight[None, :, None, None] + self.layer2_1_downsample_bn.bias[None, :, None, None] / 3
        ...
        if self.norm_bias_rule_bn == "uniform" : 
            #out_feat = (out_feat - self.layer2_2_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_2_bn1.running_var[None, :, None, None] + self.layer2_2_bn1.eps)
            out_cam = (out_cam - self.layer2_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_2_bn1.running_var[None, :, None, None] + self.layer2_2_bn1.eps)
            out_cam = out_cam * self.layer2_2_bn1.weight[None, :, None, None] + self.layer2_2_bn1.bias[None, :, None, None] / 3
            
            out_rad = (out_rad - self.layer2_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_2_bn1.running_var[None, :, None, None] + self.layer2_2_bn1.eps)
            out_rad = out_rad * self.layer2_2_bn1.weight[None, :, None, None] + self.layer2_2_bn1.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer2_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_2_bn1.running_var[None, :, None, None] + self.layer2_2_bn1.eps)
            out_psd = out_psd * self.layer2_2_bn1.weight[None, :, None, None] + self.layer2_2_bn1.bias[None, :, None, None] / 3
        ...
        # out_cam = self.layer2_2_bn1(out_cam)
        # out_rad = self.layer2_2_bn1(out_rad)
        # out_psd = self.layer2_2_bn1(out_psd)
        ...
        if self.norm_bias_rule_bn == "uniform" :
            #out_feat = (out_feat - self.layer2_2_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_2_bn2.running_var[None, :, None, None] + self.layer2_2_bn2.eps)
            out_cam = (out_cam - self.layer2_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_2_bn2.running_var[None, :, None, None] + self.layer2_2_bn2.eps)
            out_cam = out_cam * self.layer2_2_bn2.weight[None, :, None, None] + self.layer2_2_bn2.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer2_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_2_bn2.running_var[None, :, None, None] + self.layer2_2_bn2.eps)
            out_rad = out_rad * self.layer2_2_bn2.weight[None, :, None, None] + self.layer2_2_bn2.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer2_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer2_2_bn2.running_var[None, :, None, None] + self.layer2_2_bn2.eps)
            out_psd = out_psd * self.layer2_2_bn2.weight[None, :, None, None] + self.layer2_2_bn2.bias[None, :, None, None] / 3
        ...
        if self.norm_bias_rule_bn == "identity" :
            ...
            out_psd = (out_psd - self.layer2_2_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer2_2_bn2.running_var[None, :, None, None] + self.layer2_2_bn2.eps)
            out_psd = out_psd * self.layer2_2_bn2.weight[None, :, None, None] + self.layer2_2_bn2.bias[None, :, None, None]
        # out_cam = self.layer2_2_bn2(out_cam)
        # out_rad = self.layer2_2_bn2(out_rad)
        # out_psd = self.layer2_2_bn2(out_psd)
        ...
        if self.norm_bias_rule_bn == "uniform" :
            #out_feat = (out_feat - self.layer3_1_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer3_1_bn1.running_var[None, :, None, None] + self.layer3_1_bn1.eps)
            out_cam = (out_cam - self.layer3_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_bn1.running_var[None, :, None, None] + self.layer3_1_bn1.eps)
            out_cam = out_cam * self.layer3_1_bn1.weight[None, :, None, None] + self.layer3_1_bn1.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer3_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_bn1.running_var[None, :, None, None] + self.layer3_1_bn1.eps)
            out_rad = out_rad * self.layer3_1_bn1.weight[None, :, None, None] + self.layer3_1_bn1.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer3_1_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_bn1.running_var[None, :, None, None] + self.layer3_1_bn1.eps)
            out_psd = out_psd * self.layer3_1_bn1.weight[None, :, None, None] + self.layer3_1_bn1.bias[None, :, None, None] / 3
        ...
        # out_cam = self.layer3_1_bn1(out_cam)
        # out_rad = self.layer3_1_bn1(out_rad)
        # out_psd = self.layer3_1_bn1(out_psd)
        ...
        #         pdb.set_trace()
        # elif self.conservation_feat_sum : 
        #     if not torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-01, rtol=1e-01):
        #         pdb.set_trace()
        ...
        if self.norm_bias_rule_bn == "uniform":
        #out_feat = (out_feat - self.layer3_1_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer3_1_bn2.running_var[None, :, None, None] + self.layer3_1_bn2.eps)
            out_cam = (out_cam - self.layer3_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_bn2.running_var[None, :, None, None] + self.layer3_1_bn2.eps)
            out_cam = out_cam * self.layer3_1_bn2.weight[None, :, None, None] + self.layer3_1_bn2.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer3_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_bn2.running_var[None, :, None, None] + self.layer3_1_bn2.eps)
            out_rad = out_rad * self.layer3_1_bn2.weight[None, :, None, None] + self.layer3_1_bn2.bias[None, :, None, None] / 3
            
            out_psd = (out_psd - self.layer3_1_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_bn2.running_var[None, :, None, None] + self.layer3_1_bn2.eps)    
            out_psd = out_psd * self.layer3_1_bn2.weight[None, :, None, None] + self.layer3_1_bn2.bias[None, :, None, None] / 3
        ...
        #identity_feat = (identity_feat - self.layer3_1_downsample_bn.running_mean[None, :, None, None]) / torch.sqrt(self.layer3_1_downsample_bn.running_var[None, :, None, None] + self.layer3_1_downsample_bn.eps)
        
        if self.norm_bias_rule_bn == "uniform" :
            identity_cam = (identity_cam - self.layer3_1_downsample_bn.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_downsample_bn.running_var[None, :, None, None] + self.layer3_1_downsample_bn.eps)
            identity_cam = identity_cam * self.layer3_1_downsample_bn.weight[None, :, None, None] + self.layer3_1_downsample_bn.bias[None, :, None, None] / 3

            identity_rad = (identity_rad - self.layer3_1_downsample_bn.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_downsample_bn.running_var[None, :, None, None] + self.layer3_1_downsample_bn.eps)
            identity_rad = identity_rad * self.layer3_1_downsample_bn.weight[None, :, None, None] + self.layer3_1_downsample_bn.bias[None, :, None, None] / 3

            identity_psd = (identity_psd - self.layer3_1_downsample_bn.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_1_downsample_bn.running_var[None, :, None, None] + self.layer3_1_downsample_bn.eps)
            identity_psd = identity_psd * self.layer3_1_downsample_bn.weight[None, :, None, None] + self.layer3_1_downsample_bn.bias[None, :, None, None] / 3
        ...
        if self.norm_bias_rule_bn == "identity" :
            ...
            identity_psd = (identity_psd - self.layer3_1_downsample_bn.running_mean[None, :, None, None]) / torch.sqrt(self.layer3_1_downsample_bn.running_var[None, :, None, None] + self.layer3_1_downsample_bn.eps)
            identity_psd = identity_psd * self.layer3_1_downsample_bn.weight[None, :, None, None] + self.layer3_1_downsample_bn.bias[None, :, None, None]
        # identity_feat = self.layer3_1_downsample(feat_bev_)
        # identity_cam = self.layer3_1_downsample(cam_bev_)
        # identity_rad = self.layer3_1_downsample(rad_bev_)
        # identity_psd = self.layer3_1_downsample(psd_bev_)
        ...
        #         pdb.set_trace()
        # elif self.conservation_feat_sum : 
        #     if not torch.allclose(out_feat, out_cam + out_rad + out_psd, atol=1e-02, rtol=1e-01):
        #         pdb.set_trace()
        ...
        if self.norm_bias_rule_bn == "uniform":
            #out_feat = (out_feat - self.layer3_2_bn1.running_mean[None, :, None, None]) / torch.sqrt(self.layer3_2_bn1.running_var[None, :, None, None] + self.layer3_2_bn1.eps)
            out_cam = (out_cam - self.layer3_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_2_bn1.running_var[None, :, None, None] + self.layer3_2_bn1.eps)
            out_cam = out_cam * self.layer3_2_bn1.weight[None, :, None, None] + self.layer3_2_bn1.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer3_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_2_bn1.running_var[None, :, None, None] + self.layer3_2_bn1.eps)
            out_rad = out_rad * self.layer3_2_bn1.weight[None, :, None, None] + self.layer3_2_bn1.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer3_2_bn1.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_2_bn1.running_var[None, :, None, None] + self.layer3_2_bn1.eps)  
            out_psd = out_psd * self.layer3_2_bn1.weight[None, :, None, None] + self.layer3_2_bn1.bias[None, :, None, None] / 3
        ...
        # out_cam = self.layer3_2_bn1(out_cam)
        # out_rad = self.layer3_2_bn1(out_rad)
        # out_psd = self.layer3_2_bn1(out_psd)
        ...
        if self.norm_bias_rule_bn == "uniform" : 
            #out_feat = (out_feat - self.layer3_2_bn2.running_mean[None, :, None, None]) / torch.sqrt(self.layer3_2_bn2.running_var[None, :, None, None] + self.layer3_2_bn2.eps)
            out_cam = (out_cam - self.layer3_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_2_bn2.running_var[None, :, None, None] + self.layer3_2_bn2.eps)
            out_cam = out_cam * self.layer3_2_bn2.weight[None, :, None, None] + self.layer3_2_bn2.bias[None, :, None, None] / 3

            out_rad = (out_rad - self.layer3_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_2_bn2.running_var[None, :, None, None] + self.layer3_2_bn2.eps)
            out_rad = out_rad * self.layer3_2_bn2.weight[None, :, None, None] + self.layer3_2_bn2.bias[None, :, None, None] / 3

            out_psd = (out_psd - self.layer3_2_bn2.running_mean[None, :, None, None] / 3) / torch.sqrt(self.layer3_2_bn2.running_var[None, :, None, None] + self.layer3_2_bn2.eps)
            out_psd = out_psd * self.layer3_2_bn2.weight[None, :, None, None] + self.layer3_2_bn2.bias[None, :, None, None] / 3
        ...
        # out_cam = self.layer3_2_bn2(out_cam)
        # out_rad = self.layer3_2_bn2(out_rad)
        # out_psd = self.layer3_2_bn2(out_psd)
        ...
        #cam_bev_, mean_1, var_1 = self.up3_skip(cam_bev_, skip_x['3'], norm_mask)
        ...
        if self.splitting_rule == "rule_1" : 
            rad_bev_, cam_bev_, psd_bev_ = self.splitting(cam_bev_, rad_bev_, psd_bev_)
        elif self.splitting_rule == "rule_2" :
            rad_bev_, cam_bev_, psd_bev_ = self.splitting_2(cam_bev_, rad_bev_, psd_bev_) 
        ...
        if norm_mask is not None :
            if self.norm_bias_rule == "uniform":
                # feat_bev_ = (feat_bev_ - norm_mask['norm_feat_2_mean']) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                # cam_bev_ = (cam_bev_ - norm_mask['norm_feat_2_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                # rad_bev_ = (rad_bev_ - norm_mask['norm_feat_2_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                # psd_bev_ = (psd_bev_ - norm_mask['norm_feat_2_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                # if not torch.allclose(norm_mask['norm_feat_2_mean'], torch.mean(feat_bev_, dim=(2,3), keepdim=True), atol=1e-02):
                #     pdb.set_trace()
                feat_bev_ = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                cam_bev_ = (cam_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                rad_bev_ = (rad_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                psd_bev_ = (psd_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
                ...
                # if not torch.allclose(norm_mask['norm_feat_2_mean'], cam_mean + rad_mean + pseudo_mean, atol=1e-02):
                #     pdb.set_trace()
                feat_bev_ = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_2_var']+1e-7))
        ...
        # if self.conservation_feat_sum : 
        #     if not torch.allclose(feat_bev_, cam_bev_ + rad_bev_ + psd_bev_, atol=1e-01, rtol=1e-02):
        #         pdb.set_trace()
        ...
        #cam_bev_, mean_2, var_2 = self.up2_skip(cam_bev_, skip_x['2'], norm_mask)
        ...
        if norm_mask is not None :
            if self.norm_bias_rule == "uniform":
                # feat_bev_ = (feat_bev_ - norm_mask['norm_feat_3_mean']) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                # cam_bev_ = (cam_bev_ - norm_mask['norm_feat_3_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                # rad_bev_ = (rad_bev_ - norm_mask['norm_feat_3_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                # psd_bev_ = (psd_bev_ - norm_mask['norm_feat_3_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                # if not torch.allclose(norm_mask['norm_feat_3_mean'], torch.mean(feat_bev_, dim=(2,3), keepdim=True), atol=1e-02):
                #     pdb.set_trace()
                feat_bev_ = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                cam_bev_ = (cam_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                rad_bev_ = (rad_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
                psd_bev_ = (psd_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))

            elif self.norm_bias_rule == "ratio":
                cam_mean = torch.mean(cam_bev_, dim=(2,3), keepdim=True)
                rad_mean = torch.mean(rad_bev_, dim=(2,3), keepdim=True)
                pseudo_mean = torch.mean(psd_bev_, dim=(2,3), keepdim=True)
                # if not torch.allclose(norm_mask['norm_feat_3_mean'], cam_mean + rad_mean + pseudo_mean, atol=1e-02, rtol=1e-02):
                #     pdb.set_trace()
                feat_bev_ = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_3_var']+1e-7))
        ...
        # cam_bev_, mean_3, var_3 = self.up1_skip(cam_bev_, skip_x['1'], norm_mask)
        ...
        if norm_mask is not None :
            if self.norm_bias_rule == "uniform" :
                # feat_bev_ = (feat_bev_ - norm_mask['norm_feat_4_mean']) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                # cam_bev_ = (cam_bev_ - norm_mask['norm_feat_4_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                # rad_bev_ = (rad_bev_ - norm_mask['norm_feat_4_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                # psd_bev_ = (psd_bev_ - norm_mask['norm_feat_4_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                # if not torch.allclose(norm_mask['norm_feat_4_mean'], torch.mean(feat_bev_, dim=(2,3), keepdim=True), atol=1e-02):
                #     pdb.set_trace()
                feat_bev_ = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                cam_bev_ = (cam_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                rad_bev_ = (rad_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
                psd_bev_ = (psd_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) /3) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))

                # torch.sqrt로 나누게 되면서 numerical error가 점점 scale up 되는 효과가 생긴다. 

            elif self.norm_bias_rule == "ratio":
                cam_mean = torch.mean(cam_bev_, dim=(2,3), keepdim=True)
                rad_mean = torch.mean(rad_bev_, dim=(2,3), keepdim=True)
                pseudo_mean = torch.mean(psd_bev_, dim=(2,3), keepdim=True)
                # if not torch.allclose(norm_mask['norm_feat_4_mean'], cam_mean + rad_mean + pseudo_mean, atol=1e-02, rtol=1e-02):
                #     pdb.set_trace()
                feat_bev_ = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_4_var']+1e-7))
        ...
        if bev_flip_indices is not None:
            ...
            cam_bev_[bev_flip2_index] = torch.flip(cam_bev_[bev_flip2_index], [-2]) # note [-2] instead of [-3], since Y is gone now
        ...
        #segmentation_output = self.seg_conv2(self.seg_act(self.seg_instance_norm(self.seg_conv1(cam_bev_))))
        ...
        # if self.conservation_act_sum:
        #     if torch.allclose(feat_bev_, cam_bev_ + rad_bev_ + psd_bev_, atol=1e-02, rtol=1e-05) == False:
        #         pdb.set_trace()
        # elif self.conservation_feat_sum :
        #     if torch.allclose(feat_bev_, cam_bev_ + rad_bev_ + psd_bev_, atol=1e-01, rtol=1e-01) == False:
        #         pdb.set_trace()

        if norm_mask is not None :
            if self.norm_bias_rule == "uniform" : 
                # if not torch.allclose(norm_mask['norm_feat_5_mean'], torch.mean(feat_bev_, dim=(2,3), keepdim=True), atol=1e-02):
                #     pdb.set_trace()
                seg_in_feat = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt(norm_mask['norm_feat_5_var']+1e-7)
                seg_in_cam = (cam_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) / 3) / torch.sqrt(norm_mask['norm_feat_5_var']+1e-7)
                seg_in_rad = (rad_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) / 3) / torch.sqrt(norm_mask['norm_feat_5_var']+1e-7)
                seg_in_psd = (psd_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True) / 3) / torch.sqrt(norm_mask['norm_feat_5_var']+1e-7)
                
            elif self.norm_bias_rule == "ratio":
                cam_mean = torch.mean(cam_bev_, dim=(2,3), keepdim=True)
                rad_mean = torch.mean(rad_bev_, dim=(2,3), keepdim=True)
                pseudo_mean = torch.mean(psd_bev_, dim=(2,3), keepdim=True)
                # if not torch.allclose(norm_mask['norm_feat_5_mean'], cam_mean + rad_mean + pseudo_mean, atol=1e-01, rtol=1e-01):
                #     pdb.set_trace()
                seg_in_feat = (feat_bev_ - torch.mean(feat_bev_, dim=(2,3), keepdim=True)) / torch.sqrt((norm_mask['norm_feat_5_var']+1e-7))
            ...
            mean_seg = torch.mean(cam_bev_, dim=(2,3), keepdim=True)
            var_seg = torch.var(cam_bev_, dim=(2,3), unbiased=False, keepdim=True)
            self.norm_buffer.append(mean_seg)
            self.norm_buffer.append(var_seg)
            seg_in_cam = (cam_bev_ - mean_seg) / torch.sqrt(var_seg+1e-7)
        
        if self.splitting_rule == "rule_1" : 
            seg_in_rad, seg_in_cam, seg_in_psd = self.splitting(seg_in_cam, seg_in_rad, seg_in_psd)
        if self.splitting_rule == "rule_2" : 
            seg_in_rad, seg_in_cam, seg_in_psd = self.splitting_2(seg_in_cam, seg_in_rad, seg_in_psd)
        ...
        # if self.conservation_act_sum:
        #     if not torch.allclose(seg_out_feat, seg_out_cam + seg_out_rad, atol=1e-02, rtol=1e-05):
        #         pdb.set_trace()
        # elif self.conservation_feat_sum : 
        #     if not torch.allclose(seg_out_feat, seg_out_cam + seg_out_rad + seg_out_psd, atol=1e-01, rtol=1e-01):
        #         pdb.set_trace()
        ...
        #segmentation_output = self.segmentation_head(cam_bev_)
        
        # if self.conservation_feat_sum :
        #     diff = segmentation_output_feat - (segmentation_output_cam + segmentation_output_rad + segmentation_output_psd - 2 * final_bias)
        #     if not torch.allclose(segmentation_output_feat, segmentation_output_cam + segmentation_output_rad + segmentation_output_psd - 2 * final_bias, atol=1e-01, rtol=1e-05):
        #         idx = (diff.abs() > 1e-1).nonzero(as_tuple=True)
        #         if idx[0].numel() > 0: 
        #             first_idx = idx[0][0] 
        #             print(f"First large difference at index {first_idx}: {diff[first_idx]}")
        #             pdb.set_trace()
                    # raise AssertionError(f"Conservation Mismatch_2, large differences at index {first_idx}")
...
class Encoder_eff(nn.Module):
    ...
    def forward(self, cam_bev_):
        cam_bev_ = self.get_features(cam_bev_)  # get feature vector
        cam_bev_ = self.depth_layer(cam_bev_)  # feature and depth head
        return cam_bev_
...
class Segnet_relu_feats_bias(nn.Module):
    ...
    def splitting_2(self, cam_feat, rad_feat, pseudo_feat):
        ...
        # if self.conservation_bias_split : 
        #     #pdb.set_trace()
        #     assert torch.allclose(output_feat_1 + output_feat_2 + output_feat_3, cam_feat + rad_feat + pseudo_feat ,atol=1e-02, rtol=1e-05), f"Conservation Mismatch_3"
    ...
    def splitting(self, cam_feat, rad_feat, pseudo_feat):
        ...
        if check == "ratio" :
            ...
            # output_feat_2 = torch.where(ratio_condition, cam_feat + pseudo_feat * (1-ratio), output_feat_2)
            # output_feat_3 = torch.where(ratio_condition, torch.zeros_like(pseudo_feat), output_feat_3)
            sum_condition_cam = ((cam_feat > 0) & (rad_feat <= 0) & (pseudo_feat > 0)).any() or ((cam_feat < 0) & (rad_feat >= 0) & (pseudo_feat < 0)).any()
            sum_condition_rad = ((cam_feat <= 0) & (rad_feat > 0) & (pseudo_feat > 0)).any() or ((cam_feat >= 0) & (rad_feat < 0) & (pseudo_feat < 0)).any()
        ...
        if self.use_radar:
            ...
            if not self.use_metaradar:
                ...
                if norm_mask is not None:
                    #pdb.set_trace()
                    if self.norm_bias_rule == "uniform":
                        # if not torch.allclose(torch.mean(feat_bev_, dim=(2,3), keepdim=True), norm_mask['norm_feat_1_mean'], atol=1e-02):
                        #     pdb.set_trace()
                        # feat_bev_ = (feat_bev_ - norm_mask['norm_feat_1_mean']) / torch.sqrt((norm_mask['norm_feat_1_var']+1e-7))
                        # radar_bev_ = (radar_bev_ - norm_mask['norm_feat_1_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_1_var']+1e-7))
                        # camera_bev_ = (camera_bev_ - norm_mask['norm_feat_1_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_1_var']+1e-7))
                        # pseudo_bev_ = (pseudo_bev_ - norm_mask['norm_feat_1_mean'] / 3) / torch.sqrt((norm_mask['norm_feat_1_var']+1e-7))     
                        # if not torch.allclose(norm_mask['norm_feat_1_mean'], torch.mean(feat_bev_, dim=(2,3), keepdim=True), atol=1e-02, rtol=1e-02):
                        #     pdb.set_trace()
                        ...
                        # if not torch.allclose(norm_mask['norm_feat_1_mean'], cam_mean + rad_mean + pseudo_mean, atol=1e-02):
                        #     pdb.set_trace()
                        ...
                        # pdb.set_trace()
                        # print("here?")
                        # if not torch.allclose(feat_bev_, camera_bev_ + radar_bev_ + pseudo_bev_, atol=1e-02, rtol=1e-02):
                        #     pdb.set_trace()
                ...
                # if self.conservation_act_sum:
                #     # if not torch.allclose(feat_bev_, camera_bev_ + radar_bev_, atol=1e-02, rtol=1e-05):
                #         # pdb.set_trace()
                ...
                # if self.conservation_feat_sum : 
                #     assert torch.allclose(feat_bev_, camera_bev_ + radar_bev_ + pseudo_bev_, atol=1e-02), f"Conservation Mismatch_2"
            ...
            if norm_mask is not None:
                ...
                if self.norm_bias_rule == "uniform":
                    ...
                    # if not torch.allclose(norm_mask['norm_feat_1_mean'], cam_mean + rad_mean + pseudo_mean, atol=1e-02, rtol=1e-02):
                    #     # pdb.set_trace()
            ...
            # if self.conservation_feat_sum : 
            #     assert torch.allclose(feat_bev_, camera_bev_ + radar_bev_ + pseudo_bev_, atol=1e-02, rtol=1e-02), f"Conservation Mismatch_2"
