...
# xyz_ref as radar data
# feats as radar meta data
    
    def voxelize_xyz_and_feats(self, xyz_ref, feats, Z, Y, X, already_mem=False, assert_cube=False, clean_eps=0):
        B, N, D = list(xyz_ref.shape) # [1, 2100, 3]
        B2, N2, D2 = list(feats.shape) # [1, 2100, 16]
        assert(D==3)
        assert(B==B2)
        assert(N==N2)
        if already_mem:
        ...
        return feats # get feature voxels [1, 16, 200, 8, 200] 
    ...
    def get_feat_occupancy(self, xyz, feat, Z, Y, X, clean_eps=0, xyz_zero=None):
        ...
        # take valid radar points and followed features
